...
namespace Wpf.ViewModels
{
    public class VehicleViewModel : ViewModelBase
    {
        ...
        private bool ValidateVehicle()
        {
            ...
            else if (Vehicles.Any(v => v.PlateNumber == SelectedVehicle.PlateNumber && v != SelectedVehicle))
            {
                PlateNumberError = "Biển số xe đã tồn tại";
                isValid = false;
            }
            else if (
            ...
            else if (Vehicles.Any(v => v.EngineNumber == SelectedVehicle.EngineNumber && v != SelectedVehicle))
            {
                EngineNumberError = "Số máy đã tồn tại";
                isValid = false;
            }
            else if (
                !System.Text.RegularExpressions.Regex.IsMatch(
        ...
        {
            var vehicles = _vehicleService.GetVehiclesByOwnerId(SessionManager.CurrentUser.UserId);
        ...
        private void FilterVehiclesByOrnerId()
        {
            ...
            var filteredVehicles = _vehicleService
                .GetVehiclesByOwnerId(SessionManager.CurrentUser.UserId)
                .Where(v =>
                    v.PlateNumber.Contains(SearchText, StringComparison.OrdinalIgnoreCase)
                    || v.Brand.Contains(SearchText, StringComparison.OrdinalIgnoreCase)
                    || v.Model.Contains(SearchText, StringComparison.OrdinalIgnoreCase)
        ...
        private void ExecuteAdd()
        {
            ...
            SelectedVehicle.OwnerId = SessionManager.CurrentUser.UserId;
            IsEditing = true;
        }
